...
class ResourceStorage(Node):
    ...
    def __init__(self, node_id: str, resource_type: str, amount: float, max_storage: Optional[float] = None, **kwargs) \
        ...
        self._description = "This device stores a single resource, which can be used by any connected device. " \
                            "It can only store a limited number of resources. It can also only provide max amount of " \
                            "it's resource during a certain interval. This will decrease if the device is damaged"
